...
export const getBranches = createAsyncThunk(
  ...
  async (Id, thunkAPI) => {
    const { rejectWithValue } = thunkAPI;
    try {
      const result = await fetch(`${Url}branches?id=${Id}`);
      const data = await result.json();
...
export const getClientDetails = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    ...
    try {
      const data = await axios
        .post(`${Url}rest/test.branch/branche/`, {
          id,
        })
        .then((res) => res.data);
      return data;
...
export const getLastBranches = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    const { rejectWithValue } = thunkAPI;
    try {
      const res = await fetch(`${Url}rest/test.branch/last/0/`);
...
export const getLastViews = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    const { rejectWithValue } = thunkAPI;
    try {
      const res = await fetch(`${Url}rest/test.branch/viewed/9/0/`);
      const data = await res.json();
      return data;
    } catch (error) {
      return rejectWithValue(error.message);
...
export const getMatchBranch = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    ...
    try {
      const data = await axios
        .post(`${Url}rest/test.branch/matches/`, {
          id,
        })
        .then((res) => res.data);
      return data;
